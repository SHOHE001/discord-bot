...
const STATE_FILE_PATH = resolve(DATA_DIR, "tech-blog.json");
...
async function loadState(): Promise<StateMap> {
  try {
    ...
    console.error("[tech-blog] 状態ファイルの読み込みに失敗。空で続行します:", err);
    return {};
  }
}
...
export async function checkTechBlog(client: Client): Promise<void> {
  const channelId = getPluginChannelId("tech-blog");
  if (!channelId) {
    console.warn("[tech-blog] チャンネルIDが取得できませんでした");
    return;
  }

  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!(channel instanceof TextChannel)) {
    console.warn("[tech-blog] テキストチャンネルを取得できませんでした");
    return;
  }
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "rejected") {
      console.error(`[tech-blog] ${FEEDS[i].label} フィード取得失敗:`, result.reason);
      continue;
    }

    const { source, items } = result.value;
    if (items.length === 0) {
      console.log(`[tech-blog] ${source.label}: アイテムなし`);
      continue;
    }
    ...
    if (!prev) {
      console.log(`[tech-blog] ${source.label}: 初回のため通知スキップ（${allIds.length}件を記録）`);
    ...
    if (newItems.length === 0) {
      console.log(`[tech-blog] ${source.label}: 新着なし`);
      continue;
    }
    ...
    const sent = await channel.send({ embeds: [embed] }).catch((err: unknown) => {
      console.error(`[tech-blog] ${source.label} Discord送信失敗:`, err);
      return null;
    });
    ...
    if (sent) {
      ...
      console.log(`[tech-blog] ${source.label}: ${newItems.length}件通知`);
    }
  }

  await saveState(nextState).catch((err: unknown) => {
    console.error("[tech-blog] 状態ファイルの保存に失敗:", err);
  });
}
